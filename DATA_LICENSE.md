...
sources. OpenAlex and Semantic Scholar data should be reused according to their
...
- No Semantic Scholar response data is currently published. If enabled, S2-derived
  bindings and snapshots will be expressly excluded from this project's CC0
  dedication, labeled with the applicable S2 terms or written authorization, and
  accompanied by required attribution and link-back.

Changing the repository's data notice alone cannot create permission to redistribute
Semantic Scholar response data. The maintainer must first confirm that the intended
static display and downloadable historical snapshots are authorized.
