...
export function Methodology({ years = [] }: { years?: number[] }) {
  ...
  return <article className="methodology">
    ...
    <section><span>05</span><div>
      ...
      <p>{zh ? <>
        Google Scholar、OpenAlex 与 Semantic Scholar 的收录范围、版本聚合、去重规则和更新时间并不相同，因此同一论文的引用量可能明显不同，也不存在跨平台统一的“真实计数”。本站在有经过核验的 Scholar 快照时默认展示 Google Scholar 观测：优先通过 SerpApi 获取，在配额或服务不可用时可使用经过验证的 ScraperAPI 后备，并记录实际传输服务。ScraperAPI 后备仅提供当前总数，不提供逐年分布。各来源的数值、标识和时间戳保持独立，不相加。Semantic Scholar 观测作为单独署名的数据序列发布，并明确排除在项目的 CC0 声明之外；相关学术成果请参阅 <a href="https://arxiv.org/abs/2301.10140" target="_blank" rel="noreferrer">Kinney 等人的《The Semantic Scholar Open Data Platform》↗</a>。
      </> : <>
        Google Scholar, OpenAlex, and Semantic Scholar differ in coverage, version clustering, deduplication, and update cadence, so the same paper can have substantially different counts; there is no provider-independent “true count.” When verified Scholar snapshots exist, this site defaults to Google Scholar observations. It prefers SerpApi and can use a verified ScraperAPI fallback when quota or service availability requires it, recording the actual transport. ScraperAPI fallback observations provide the current total but not a citing-year distribution. Counts, identifiers, and timestamps remain separate and are never added together. Semantic Scholar observations are published as a separately attributed series and remain outside the project's CC0 dedication; see <a href="https://arxiv.org/abs/2301.10140" target="_blank" rel="noreferrer">Kinney et al., “The Semantic Scholar Open Data Platform” ↗</a>.
      </>}</p>
    </div></section>
